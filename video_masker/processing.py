...
def mask_faces_selective(frame, analyzer, person_embeddings, enabled, masker,
                         margin=0.0, match_threshold=0.75):
    """人物選択モードの顔マスク。

    各顔を、人物ごとのメンバー埋め込み集合への最小距離で最近傍クラスタに割り当て、
    しきい値内で一致したクラスタの ON/OFF に従う。
    - OFF（隠さない）人物に一致 → 隠さない
    - ON の人物 or どのクラスタにも遠い（未知）→ 隠す（安全側）
    重心一点ではなくメンバー集合との最小距離で判定するため、同一人物の角度・
    表情の変化に強く、「隠さない人なのに隠れる」誤マスクを減らす。
    """
    from video_masker.face_clustering import assign_to_clusters

    for face in analyzer.analyze(frame):
        idx, dist = assign_to_clusters(face["embedding"], person_embeddings)
...
def mask_frame(
    ...
    if use_faces:
        if (analyzer is not None and person_embeddings is not None
                and person_enabled is not None):
            # 人物選択モード（insightface で検出＋照合し ON の人物だけ隠す）
            mask_faces_selective(frame, analyzer, person_embeddings,
                                 person_enabled, masker, face_margin)
        else:
...
def process_video(
    ...
    use_person_select = bool(use_faces and person_embeddings is not None
                             and person_enabled is not None)
    analyzer = None
...
def process_image(
    ...
    if use_person_select:
        ...
        analyzer = FaceAnalyzer(log_cb)
        if analyzer.usable:
            mask_faces_selective(image, analyzer, person_embeddings,
                                 person_enabled, masker, face_margin)
        else:
            use_person_select = False
